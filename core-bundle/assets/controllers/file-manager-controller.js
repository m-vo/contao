...
export default class FileManagerController extends Controller {
    ...
    connect() {
        this.dropzone = new Dropzone(this.dropzoneTarget, {
            ...
            clickable: false,
            // hiddenInputContainer: this.dropzoneTarget,
            // acceptedFiles
        });
    ...
    dragEnter(event) {
        ...
        if (event.target.dataset.resource !== this._getCommonBasePath(this._draggedPaths)) {
    ...
    dragDrop(event) {
        ...
        if (event.dataTransfer.getData('contao/file-manager-paths')) {
            // move with this._draggedPaths, path
            this.actionStreamConnection.get(this.operationUrl, {});

            return;
        }

        // Drag and drop from external: initiate a dropzone upload
        // todo: uploadUrl -> uploadAction with path
